...
export default class MapboxLegendControl implements IControl {
  ...
  constructor(
    targets: { [key: string]: RasterLayerOpts | VectorLayerOpts },
  ...
  private changeLayerVisibility(layer_id: string, checked: boolean) {
  ...
  private createLayerCheckbox(layer: mapboxgl.Layer): HTMLElement | undefined {
    ...
    checklayer.addEventListener("click", function (e) {
      // @ts-ignore
      //const _id = `${e.target?.value}-outline`;  // this line disables layer enabling/disabling - why?
    ...
    const afterLoadListener = async () => {
      if (map.loaded()) {
        ...
        if (styleUrl && styleUrl.includes("mapbox://")) {
          styleUrl = styleUrl.replace(/mapbox:\/\//g, baseApiUrl).replace(/sprites/g, "/styles/v1");
